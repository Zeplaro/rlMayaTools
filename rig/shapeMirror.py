    ...
    if not copy:
        ...
        for slave in slaves:
            if mc.nodeType(master) == 'nurbsCurve':  # if shapes are selected
                ...
                mastershape = tbx.getShape(master)
                slaveshape = tbx.getShape(slave)
                while len(slaveshape) > len(mastershape):
                    slaveshape.pop(-1)
                while len(slaveshape) < len(mastershape):
                    mastershape.pop(-1)
