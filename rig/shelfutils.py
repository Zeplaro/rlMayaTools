...
def get_shelf_data(shelf=None):
    ...
    for button in cmds.shelfLayout(shelf, query=True, childArray=True) or []:
        ...
        if ui_type == "separator":
            ...
            button_data = {
                kwarg: cmds.shelfButton(button, q=True, **{kwarg: True}) for kwarg in button_kwargs
            }
            button_data["menu"] = []
            for menu in cmds.shelfButton(button, q=True, pma=True) or []:
                for item in cmds.popupMenu(menu, q=True, itemArray=True) or []:
                    if cmds.menuItem(item, q=True, label=True) in excluded_labels:
                        continue
                    menu_data = {
                        kwarg: cmds.menuItem(item, q=True, **{kwarg: True}) or ""
...
def set_shelf_data(data, shelf=None):
    ...
    if not shelf:
        shelf = get_current_shelf()
    for button_data in deepcopy(
        data
    ):  # deepcopy to not alter the given data object or its values with later .pop()
        if button_data == "separator":
            ...
            button = cmds.shelfButton(
                parent=shelf,
                imageOverlayLabel=button_data["imageOverlayLabel"],
                sourceType=button_data["sourceType"],
                image=button_data["image"],
                label=button_data["label"],
                command=button_data["command"],
                annotation=button_data["annotation"],
                doubleClickCommand=button_data["doubleClickCommand"],
            )
            ...
            for menu_data in menu_items:
                ...
                cmds.menuItem(
                    parent=menu,
                    command=menu_data["command"],
                    sourceType=menu_data["sourceType"],
